---
title: "Environmental effects on reference points"
...
-   CRAN: [glmmTMB](https://cran.r-project.org/web/packages/glmmTMB/), [earth](https://cran.r-project.org/web/packages/earth), [plyr](https://cran.r-project.org/web/packages/plyr), [stringr](https://cran.r-project.org/web/packages/stringr/), [reshape2](https://cran.r-project.org/web/packages/reshape2/), [ggplot2](https://cran.r-project.org/web/packages/ggplot2/), [ggdark](https://cran.r-project.org/web/packages/ggdark/), [patchwork](https://cran.r-project.org/web/packages/patchwork/)
-   FLR: [FLCore](https://flr-project.org/FLCore/), [FLFleet](https://flr-project.org/FLFleet/), [FLash](https://flr-project.org/FLash/), [FLAssess](https://flr-project.org/FLAssess/), [FLBEIA](https://flr-project.org/FLBEIA/)
...
### 3.2 Define Global Setup

Here we just define the overall setup for the MSE: what Ftarget\`s to run, what biological parameters to vary or include, the periods for which we want to evaluate the ref-pt changes as well as the number of iterations.

The aim is to run longterm simulations under varying F + a constant environmental forcing corresponding to the average of the chosen period (e.g. 2030s) until reaching equilibrium (\~2099) and take the equilibrium yield and SSB as a basis to see reference point changes.

```{r Global Setup, message = F, error=F,warning=F}
# global settings ------------------------------------------------------

# define where to include uncertainty 
# (sr - stock recruitment,bio - biological params., sel - selectivity)
stochastic_sr <- TRUE
stochastic_bio <- TRUE
stochastic_sel <- TRUE
...
### 3.3 Load pre-Conditioned Input data

Since we do not want to go all the way through the conditioning of the modell, we just load the pre-conditioned MSE input and modify it accordingly to match our simulations.
...
if(stk_name == "COD-NS"){
  biols[[nameChange(stk_name,"new")]]@m[ac(3:RANGE["max"]), ac(2011:RANGE["maxyear"])] <- 
    biols[[nameChange(stk_name,"new")]]@m[ac(3:RANGE["max"]), ac(2011:RANGE["maxyear"])] + log(1-0.15)
}
```
...
$logw_{t+1} = logw_{t} + age + I(age^2) + age:logw + I(age^2):logw + sal.s + temp.s + (1 | cohortf) +  (1 | yearf)$
...
In order to have a "baseline" to compare to, we generate a 'noCC' or 'Current' climate scenario, where we remove the trend from the RCP4.5 scenario. Based on RCP-scenarios, we prepare also the environmental data corresponding to the mean of the periods 2030, 2040 and 2050 that we use to inform the simulations in order to get reference point changes for those periods.
...
```{r Prepare the growth Covars,message=F,warning=F,error=FALSE}
  ...
  str(Stock_ICES.avg_projections,
      max.level = 2)
...
### 3.5 Uncertainty Setup

Here we define the resampling schemes for different biological parameters.

Add uncertainty in the EMSRR via resampling of the EMSRR-model + uncertainty in the environmental covariates.
...
### 3.6 Env. Period Setup

As we did for the growth model already, we also need to setup the environmental data used by the stock-recruitment relationship in those periods (2030, 2040, 2050) and RCPs. The stock recruitment relationship for cod uses only temperature (PC1 of North Sea SST) as covariate.
...
## 4. FLBEIA simulations

### 4.1 Setup simulation function

Definition of the simulation function that gets executed in parallel: Here we query the scenario table `scen` and based on that modify the inputs of our simulations. Uncertainty (both resampled and environmental) is included for each iteration in a Monte-Carlo fashion.
...
Now we can simulate for the pre-defined scenarios. However, as this is quite computational expensive (\~12h on 20 server-cores for 100 iterations), we skip this here and only look at the prepared output. The code is here for completeness.
...
We now summarise the results and calculate the changes in Ref-pts (in terms of Btrigger and Fmsy) for the different periods.

Therefore, we calculate the median ssb and landings in the equillibrium (last 10 years of the simulation) of each iteration and fit a smooth-spline to the respective yield curve (landings\~F) and biomass curve (ssb\~F). The F, where the maximum of the yield-curve lies is taken as Fmsy. Bmsy is calculated as the SSB at Fmsy and Btrigger respectively as the 5% quantile of the SSB when fishing at this Fmsy.
...
Now we visualize the output, plotting the yield curves and evaluate how climate change might affect reference points if explicit environmental effects are included to affect growth and recruitment of North Sea cod.

```{r Analyse output,warning=F,error=F,fig.width=10,fig.height=6}
...
p <- ggplot(data = dfref2) + 
  ...
  ggtitle(paste("Ref-pts. changes",stk_name)) +
  dark_theme_gray()
...
So what we see here is that cod is negatively affected by climate change (mainly due to the negative effect of temperature on recruitment) and this results in decreasing reference points for Fmsy, Bmsy and Btrigger. The decrease is even stronger for the RCP8.5 scenario than the RCP4.5 scenario, although the differences are not that pronounced until mid-century. In the 2050s the deviations between the two RCPs get stronger and as such also the difference in reference points.

```{r Plot yield curves,message=F,warning=F,error=F}
# yield curve --------------------------------------------------------
...
With this tutorial we have learned:

-   how to estimate reference points in a MSE framework (with FLBEIA)
-   how productivity changes under climate change might affect reference points and how we could simulate it

A next step would be to use those results to inform forward projections under different harvest control rules explicitly accounting for changes in reference points. Therefore we would mimic in a way the constant updating of reference points in the ICES framework and see if longer term management strategy plans are also robust in regards to those reference point changes. A further step would be to transfer those into the mixed fisheries framework, which is what we usually use FLBEIA for and see the effect on several stocks with included productivity changes on typical mixed fisheries questions e.g. technical interactions (choking effects).
...
**ggdark:** `r packageVersion('ggdark')`

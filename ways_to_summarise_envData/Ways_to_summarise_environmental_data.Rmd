...
install.packages( c("raster","terra","maps","sf","kohonen","corrplot","PCDimension"))
...
```{r Load data and prepare data, message = F}
# load data
...
NA.mask = union(union(bind(rm.IrishS1,rm.IrishS2,rm.EngCh),bind(rm.Kattg1,rm.Kattg2)),spatial.domain)#,"SpatialPolygonsDataframe")
...
# cut 
AHOI_SST_raster = mask(crop(AHOI_SST_raster,extent(c(-10.1,12,49,64))),NA.mask,inverse = T,updatevalue = NA)
...
ICES.areas_lat.lon_sub = ICES.areas_lat.lon[ICES.areas_lat.lon$Area_Full %in% 
                         paste(27,c("3.a.20","4.a","4.b","4.c","6.a","6.b","7.d"),sep = "."),]
...
```{r Spatial averages 3,message = F,results = 'hide',warning=F,error=F,fig.width=5,fig.height=6}
...
```{r Compare the 3 averaging procedures,message = F,results = 'hide',warning=F,error=F,fig.width=7,fig.height=5}
# compare
...
```{r EOF analysis,message = F,warning=F,error=F}
...
plot(eof.out_YR$sp.PCA$sdev^2/sum(eof.out_YR$sp.PCA$sdev^2),type = "h",xlab = "PC",ylab = "expl. Var")
...
eot.out_YR = fast.eot(x = temp.YR_centered,n_cores = 4,n = n.eots, standardised = F)
...
find.elbow = function(modes,expl.var,plot = F){
  ...
  if(plot == T){
    plot(df$mode,df$expl.var,type = "p",pch = 16,xlab = "mode",ylab = "Cum. expl. Var [%]")
    lines(seq(0,max(modes),0.01),
          predict(mod,data.frame(mode = seq(0,max(modes),0.01))),lwd = 2,col = "gray50")
...
tSOM_temp = temporal.SOM(x = temp.YR,time = 1948:2020,plot = T,parallel = c(parallel = T,cores = 4),seed = 42,reps = 100,
                         grid = kohonen::somgrid(xdim = 2, ydim = 3, topo = "rectangular",
...
layout(matrix(c(1:3,rep(5,2),rep(4,5)),nrow = 2,ncol = 5,byrow = T),height = c(1.2,0.8))

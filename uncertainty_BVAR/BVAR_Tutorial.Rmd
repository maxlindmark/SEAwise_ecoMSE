...
library(raster)
library(terra)
library(marmalaid) # devtools::install_github("BernhardKuehn/marmalaid")
library(BVAR)
library(scam)
library(mgcv)
library(funtimes)
...
par(mfrow = c(1,1))
plot(EOF.Salinity$time,y.sim_list[[1]][,1],col = "gray50",type = "l",ylab = "",xlab = "time",
     ylim = range(y.sim_list[[1]][,1])*1.5)
title("Comparison of obs. vs. sim. time series",adj = 0)
for(i in 1:100){
  lines(EOF.Salinity$time,y.sim_list[[i]][,1],col = "gray70")  
}
lines(EOF.Salinity$time,y.trend.rm[,1],col = "black",lwd = 1.7)
legend("topleft",c("observed","simulated"),col = c("black","gray70"),bty = "n",lwd = 2)
# check acf
...
for(kk in 1:nrow(lut)){
  ...
  plot.list[[n]] = ggplot(ccfs_quant) + 
    ...
    geom_point(data = df.ccf.obs,
               aes(x = lag,y = ccf),col = "black") +
    geom_segment(data = df.ccf.obs,
                 aes(x = lag,xend = lag,y = ccf,yend = 0),col = "black") + 
...
for(i in 1:10){
  lines(cellStats(brick(BVAR_wrapper_sims$sims$Salinity[[i]]),mean),col = "gray50")
}
lines(cellStats(POLCOM_ERSEM_Salinity_raster_meanBiasRM_anom$rcp45,mean),col = "black")
title("Salinity anom.",adj = 0)

plot(cellStats(POLCOM_ERSEM_SST_raster_meanBiasRM_anom$rcp45,mean),
     type = "l",xlab = "",ylab = "")
for(i in 1:10){
  lines(cellStats(brick(BVAR_wrapper_sims$sims$SST[[i]]),mean),col = "gray50")
}
lines(cellStats(POLCOM_ERSEM_SST_raster_meanBiasRM_anom$rcp45,mean),col = "black")
title("SST anom.",adj = 0)
plot(cellStats(POLCOM_ERSEM_Currents_raster_meanBiasRM_anom$rcp45$u,mean),
     type = "l",xlab ="",ylab = "")
for(i in 1:10){
  lines(cellStats(brick(BVAR_wrapper_sims$sims$Currents[[i]][[1]]),mean),col = "gray50")
}
lines(cellStats(POLCOM_ERSEM_Currents_raster_meanBiasRM_anom$rcp45$u,mean),col = "black")

...
Another consideration is the  forecast horizon of interest, which is a main consideration of the test data set size. Thus, the length of both holdout sets should not differ too much, otherwise you may choose a model based on prediction at one time scale, while evaluating prediction performance at another. 
...
#### Example of hindcast with synthetic data
...
__Compare prediction of test set to naive model__
...
### Another hindcast test - peels

A strategy that is slowly being adopted in the selection of stock assessment models, is to see how robust they are to prediction 
...
p1 <- ggplot(data = dfTrain) + aes(x = t, y = yhat_mod, color = peel, group = peel) + 
  ...
  labs(x = NULL, y = "recruitment", color = "Title") +
  annotate("text", x = min(res$t), y = max(res$rec), 

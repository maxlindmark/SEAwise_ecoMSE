...
cvFun <- function(model = NULL, nperm = 10, k = 5, seed = 1){
  ...
  for(j in seq(nperm)){ # for each permutation j
    ks <- kfold(n = nrow(model$data), k = k)
    res[[j]] <- data.frame(perm = j, k = NaN, y = model$y, yhat = NaN)
    for(i in seq(ks)){ # for each partition k
      train <- -ks[[i]]
      valid <- ks[[i]]
      fit.k <- update(object = model, data = model$data[train,]) # update model with training data
      res[[j]]$k[valid] <- i
      res[[j]]$yhat[valid] <- predict(object = fit.k, newdata = model$data[valid,], type = "response")
    }
  }
...
fitnessFun <- function(genes = NULL, data = NULL, nperm = 10, k = 5, ffSeed = 1, fitnessOnly = TRUE){
  ...
  # fmla2 <<- paste(c("rec ~ offset(log(ssb)) + ssb", envvars[as.logical(genes)]), collapse = " + ")
  # fit <- glm(formula = as.formula(fmla2), data = data, family = gaussian(link = "log"))

  fmla3 <<- paste(c("rec ~ offset(log(ssb)) + ssb", envvars[as.logical(genes)]), collapse = " + ")
  fit <- glm(formula = fmla3, data = data, family = gaussian(link = "log"))
...
par(mar = c(1,4,5,1))
image(list(y = seq(nrow(startPop)), x = seq(ncol(startPop)), z = t(startPop)),
  ylab = "Individuals", xlab = "", 
  ylim = rev(range(seq(nrow(startPop)))), xaxt = "n")
axis(3, at = seq(ncol(startPop)), labels = envvars, las = 2)
mtext("Covariates (genes)", side = 3, line = 3)
box()
...
ga.fit <- ga(
  ...
  maxiter = 50, # max number of generations
  run = 20, # the max number of generations without any improvement
  monitor = T, # change to TRUE to monitor fitness evolution
  data = dat, # input data

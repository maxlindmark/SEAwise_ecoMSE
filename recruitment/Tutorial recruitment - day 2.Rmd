...
To test the significance of environmental variables on recruitment process, first we need to select meaningful covariates. The environmental time series used in this tutorial have been derived from Copernicus (https://www.copernicus.eu/en, reanalysis approach on Bernhard presentation)
...
# 
#env_hindcast=env_hindcast[env_hindcast$YEAR>=y[1] & env_hindcast$YEAR<=y[length(y)],]
# 
...
Using the presented approach it is possible to explore any SRR model just adapting the script. It would be also possible to adapt the scipt to fit EMSRR applying a model error approach (in case a robust and contrasted dataset is available).

More complex indicators can be introduced following Marc presentation on model selection. 

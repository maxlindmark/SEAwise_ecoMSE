...
-   CRAN: [glmmTMB](https://cran.r-project.org/web/packages/glmmTMB/), [earth](https://cran.r-project.org/web/packages/earth/), [cobs](https://cran.r-project.org/web/packages/cobs/), [basetheme](https://cran.r-project.org/web/packages/basetheme/)
...
install.packages( c("glmmTMB","earth","basetheme"))
    ...
    for(j in seq_along(mts)){
      ...
      for(k in names(fleets[[flt]]@metiers[[mt]]@catches)){
        ...
        if(grepl("NEP",k)){
          ...
          wt_new = seq(
            0,abs(0.4 * max(tmp1$wt)) + max(tmp1$wt), # interpolate between 0 and 140% of max.value
            length.out = 500
          )

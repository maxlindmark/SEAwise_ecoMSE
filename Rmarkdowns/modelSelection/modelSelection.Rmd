...
In the following example, a simple linear model is evaluated by 
...
You can compare models of different structure as well. For example, AIC may not be comparible between models of different structure, e.g. mixed models with random effects. Comparison should be made using same folds (e.g. use of random seed). A variety of metrics could be used for the final model selection.
...
Depending on the number of potential candidate models, the the complexity of the model fitting and cross validation procedures, model selection can become computationally costly. 

In the example by @kuhn_using_2021, a large number of candidate covariates (n > 80) were chosen based on hypothesized bioloical mechanisms affecting recruitment of North Sea cod. These included time series signals extracted from environmental fields (e.g. principal components from temperature, salinity, and currents) at different temporal periods (and lags). 

The approach used an efficient search algorithm called *Non-dominated sorting genetic algorithm* (NSGA-II) combined with cross validation to explore the large number of potential model configurations. In the following example, we will explore a simplified version of the approach using a genetic algorithm.
...
## Flavor 3: Hindcast and naive comparison

Hindcast cross validation performance
...
ggplot(df) + aes(x = t, y = 0, col = split) + 
  geom_point() +
  scale_color_brewer(name = "Split", palette = "Dark2") +
  labs(y = "") +
  scale_y_continuous(limits = c(0, 0), breaks = NULL) + 
  theme(axis.title.y = element_blank(), axis.text.y = element_blank(),
    axis.ticks.y = element_blank(), axis.line.y = element_blank()) +
...
```{r rollingTestHoldout, fig.height=2}
n <- nrow(dat) # length of time series
nTest <- 5 # number of test holdout values
...
Finally, it may be useful to compare the prediction performance to some *naive* prediction, such as a men of recent values. The *MASE* metric is of particular use for this kind of forecast comparison.
...
The best model is `r best`, which we will then fit again to the full training/valididation data, and predict the testing holdout. The naive model prediction will be the geometric mean of recruitment from the last 10 years.
...
MASE <- mase(y = datTest$rec, yhat = yhat, yhat_naive = yhat_naive)
MASE
```
```{r naiveCompare2, fig.height=4}
df1 <- dat[,c("t", "rec")]
...
df$type <- factor(df$type, levels = c("train/valid", "test", "model", "naive"))

ggplot(df) + aes(x = t, y = rec, colour = type) + 
  geom_point() + 
  geom_line() +
  annotate("text", x = min(df$t), y = max(df$rec), 
    label = paste("MASE =", round(MASE, 3)), hjust = 0, color = "black") + 
  scale_color_manual(name = "Data type", 
    values = c("grey30", "grey", "blue", "green4")) +
  theme_bw()

```

So, the prediction perfomance is slightly better than the naive prediction (MASE < 1.0), but this might not be strong enough to use in practice. Note that the model was given future SSB and the environmental covariates
